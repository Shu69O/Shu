...
Группируем по аэропорту, времени вылета и модели самолета и нумеруем места с помощью окна
...
select f.flight_id, f.flight_no, f.departure_airport, f.status, f.actual_departure, a.model, s.seat_no,
	row_number() over (partition by f.departure_airport, f.actual_departure, a.model order by s.seat_no)
from flights f 
